...
public class NotificationHeadDao implements ICommonBusinessDao<NotificationHeadRequestTo, NotificationHeadResponse> {
    private static final Logger logger = Logger.getLogger(NotificationsDao.class);
    private IDao dao;

    public NotificationHeadDao(IDao dao) {
        this.dao = dao;
    }

    @Override
    public NotificationHeadResponse queryDao(NotificationHeadRequestTo notificationHeadRequestTo) throws FarmatodoException {
        final String transaccionId = notificationHeadRequestTo.getTransactionId();
        final String metodo = getClass().getCanonicalName() + ".queryDao";
        final String origen = notificationHeadRequestTo.getSource();
        ...
        final String usuario = notificationHeadRequestTo.getSecurity().getUser();
        final String country = notificationHeadRequestTo.getCountry();
        long tiempo = System.currentTimeMillis();
        NotificationHeadResponse responseTo = new NotificationHeadResponse();
        try (
            ...
            notificationParameter.setParameters(requestResponsePkgMain.getJsonPublicParamRequestSelectAllGet(
                    MUDULE_NAME, PRC_NOTI_TYPE_GET, notificationHeadRequestTo));
            ...
            // Toma resultado
            //---------------------------------------------------------
            responseTo = (NotificationHeadResponse) requestResponsePkgMain.getSmsResponse(stmt.getString(4), notificationHeadRequestTo);
            ...
            if (!responseTo.getCod().equals(Constants.BD_CODIGO_EXITO)) {
                throw new AdvertenciaFuncionalException(notificationHeadRequestTo, responseTo)
                ...
                NotificacionHeadList notiHeadData = gson.fromJson(stmt.getString(2), NotificacionHeadList.class);
                responseTo.setNotiHeadData(notiHeadData.getNotiHeadData());
            }
        } catch (Exception ex) {
    ...
    @Override
    public NotificationHeadResponse saveDao(NotificationHeadRequestTo notificationHeadRequestTo) throws FarmatodoException {
        final String transaccionId = notificationHeadRequestTo.getTransactionId();
        ...
        try (
            ...
            notificationParameter.setParameters(requestResponsePkgMain.getPublicParamRequestSet(
                    MUDULE_NAME, PRC_SET_NOTI_TYPE, notificationHeadRequestTo));
            notificationParameter.setNotiHead(notificationHeadRequestTo.getNotiHead());
            // Serializa a Json los parametros de entrada del
            //---------------------------------------------------------
            String jsonParametersRequest = gson.toJson(notificationParameter);
            // Pasando los parametros al Stored Procedure
    ...
    @Override
    public NotificationHeadResponse deleteDao(NotificationHeadRequestTo notificationHeadRequestTo) throws FarmatodoException {
        final String transaccionId = notificationHeadRequestTo.getTransactionId();
        final String metodo = getClass().getCanonicalName() + ".saveDao";
        final String origen = notificationHeadRequestTo.getSource();
        final String nodo = System.getProperty("NODO");
        ...
        try (
            ...
            //---------------------------------------------------------
            notificationParameter.setParameters(requestResponsePkgMain.getPublicParamRequestSet(
                    MUDULE_NAME, PRC_DEL_NOTI_TYPE, notificationHeadRequestTo));
            notificationParameter.setNotiHead(notificationHeadRequestTo.getNotiHead());
    ...
    @Override
    public NotificationHeadResponse mergeDao(NotificationHeadRequestTo notificationHeadRequestTo) throws FarmatodoException {
        return null;
    }

    @Override
    public NotificationHeadResponse updateDao(NotificationHeadRequestTo notificationHeadRequestTo) throws FarmatodoException {
        final String transaccionId = notificationHeadRequestTo.getTransactionId();
        final String metodo = getClass().getCanonicalName() + ".saveDao";
        final String origen = notificationHeadRequestTo.getSource();
        final String nodo = System.getProperty("NODO");
        ...
        try (
            ...
            //---------------------------------------------------------
            notificationParameter.setParameters(requestResponsePkgMain.getPublicParamRequestSet(
                    MUDULE_NAME, PRC_UPD_NOTI_TYPE, notificationHeadRequestTo));
            notificationParameter.setNotiHead(notificationHeadRequestTo.getNotiHead());
