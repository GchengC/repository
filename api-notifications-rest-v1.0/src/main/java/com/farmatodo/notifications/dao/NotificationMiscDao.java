...
public class NotificationMiscDao implements ICustomBusinessDao<NotificationRequestTo, NotificationResponseTo> {
    private static final Logger logger = Logger.getLogger(NotificationMiscDao.class);
    private IDao dao;

    public NotificationMiscDao(IDao dao) {
        this.dao = dao;
    }
    ...
    public NotificationResponseTo queryDao(NotificationRequestTo notificationRequestTo) throws FarmatodoException {
        ...
        try (
            ...
            notificationParameter.setParameters(requestResponsePkgMain.getJsonPublicParamRequestSelectAllGet(
                    MUDULE_NAME, PRC_NOTI_ID_GET, notificationRequestTo));
            ...
            if (!responseTo.getCod().equals(Constants.BD_CODIGO_EXITO)) {
                ...
                NotificationMiscList notiReports = gson.fromJson(stmt.getString(2), NotificationMiscList.class);
                responseTo.setNotifications(notiReports.getNotifications());
            }
    ...
    @Override
    public NotificationResponseTo getDao(NotificationRequestTo notificationRequestTo) throws FarmatodoException {
        final String transaccionId = notificationRequestTo.getTransactionId();
        final String metodo = getClass().getCanonicalName() + ".getDao";
        final String origen = notificationRequestTo.getSource();
        final String nodo = System.getProperty("NODO");
        final String tipoTransaccion = FarmatodoException.TIPO_TRANSACCION_DB;
        final String aplicacion = Constants.NOMBRE_APLICACION;
        ...
        try (
            ...
            //---------------------------------------------------------
            notificationParameter.setParameters(requestResponsePkgMain.getJsonPublicParamRequestSelectAllGet(
                    MUDULE_NAME, PRC_HEAD_ID_GET, notificationRequestTo));
            ...
            if (!responseTo.getCod().equals(Constants.BD_CODIGO_EXITO)) {
                ...
                NotificationMiscList notiHead = gson.fromJson(stmt.getString(2), NotificationMiscList.class);
                responseTo.setNotifications(notiHead.getNotiHead());
            }
        } catch (Exception ex) {
            ManejadorExcepciones.manejarYLanzar(
                    new FarmatodoException()
    ...
    @Override
    public NotificationResponseTo findDao(NotificationRequestTo notificationRequestTo) throws FarmatodoException {
        return null;
    }

    @Override
    public NotificationResponseTo setDao(NotificationRequestTo notificationRequestTo) throws FarmatodoException {
        return null;
    }
